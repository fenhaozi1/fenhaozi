...
# ---------- 烟花粒子列表 ----------
fireworks = []  # 每个元素: {x, y, vx, vy, color, life, max_life, radius}
...
def create_firework():
    ...
    base_color = random.choice([RED, GREEN, YELLOW, BLUE, (255, 180, 80), (200, 120, 255)])
...
def draw_fireworks(surf):
    ...
    for p in fireworks:
        ...
        if radius > 0:
            ...
            surf.blit(s, (int(p['x'] - radius), int(p['y'] - radius)))
...
def draw_win(is_last):
    ...
    if is_last:
        t1 = font_small.render("查看最终结果", True, WHITE)
    else:
...
while running:
    ...
    if state == STATE_PLAY:
        ...
        # 每帧有概率生成新烟花，并更新粒子
        if random.random() < 0.04:   # 约每秒 2~3 次
            create_firework()
        update_fireworks(dt)
