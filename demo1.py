...
pygame.display.set_caption("一箭又一箭 (初始版)")
...
# ---------- 方向映射（故意错误版本）----------
# 移动方向 dx, dy 是对的，但旋转角度是错的，导致箭头图形指向与飞出方向不一致
DIRS = {
    'U': (0, -1, -math.pi / 2),   # 错误：朝上却旋转 -90 度
    'D': (0, 1, math.pi / 2),     # 错误：朝下却旋转 +90 度
    'L': (-1, 0, math.pi),        # 错误：朝左却旋转 180 度
    'R': (1, 0, 0)                # 错误：朝右却旋转 0 度
}
...
# ---------- 更新逻辑 ----------
def update(dt):
    global fail_delay, state, win_timer
...
def draw_game():
    ...
    btn_restart = pygame.Rect(info_x, 300, 170, 50)
    ...
    screen.blit(font_small.render("重新开始本关", True, WHITE), (info_x+15, 313))
    return btn_restart
...
def draw_win(is_last):
    ...
    sub = font_mid.render(f"剩余失误 {mistakes_left}", True, GRAY)
...
def draw_all():
    ...
    screen.blit(title, (WIDTH//2 - title.get_width()//2, 150))
    sub = font_mid.render("你完成了所有关卡，太厉害了", True, GRAY)
    screen.blit(sub, (WIDTH//2 - sub.get_width()//2, 270))
...
def draw_fail():
    ...
    screen.blit(title, (WIDTH//2 - title.get_width()//2, 150))
    sub = font_mid.render("失误次数已用完，再试一次吧", True, GRAY)
    screen.blit(sub, (WIDTH//2 - sub.get_width()//2, 270))
